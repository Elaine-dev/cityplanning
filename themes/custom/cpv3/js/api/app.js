...
app.controller('MainCtrl', function($scope, GeneralService) {
	...
	$scope.$watch('case_number_search',function(newValue) {
		GeneralService.getAgenda('http://161.149.221.142/caseinfo/casesummary.aspx?case='+newValue+'&engine=json').then(function(response){
	...
	$scope.search = function(case_number){
		...
		GeneralService.getAgenda('http://161.149.221.142/caseinfo/casesummary.aspx?case='+case_number+'&engine=json').then(function(response){
			$scope.results = response.data;
			$scope.sections = $scope.results.sections;
			$scope.data = [];
			$scope.items = [];
