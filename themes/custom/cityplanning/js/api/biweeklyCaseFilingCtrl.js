app.controller('biweeklyCaseFilingCtrl', function($scope, biweeklyCaseFilingService, $filter, $timeout) {
	$scope.enable = "false";
	...
	*/
	...
	$scope.$watch('date', function(newAPC) {
		if (newAPC) { 
			var selectedYear = $scope.date;
			biweeklyCaseFilingService.getList(newAPC, selectedYear).then(function(response) {
				// loading icon
				$timeout(function(){
					$scope.enable = true;
					$scope.loadingText = '';
				...
				$scope.dates = [{'2017':'2017'},{'2016':'2016'},{'2015':'2015'},{'2014':'2014'}];
	...
	$scope.$watch('date', function(newYear) {
		if (newYear) {
			var apc = $scope.apc;
			biweeklyCaseFilingService.getList(newYear).then(function(response) {
				$scope.getList = response.data.Entries;
				//$scope.title = response.data.APCTitle;
